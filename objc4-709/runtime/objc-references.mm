...
id _object_get_associative_reference(id object, void *key) {
    ...
    uintptr_t policy = OBJC_ASSOCIATION_ASSIGN;
    {
        ...
        if (i != associations.end()) {
            ...
            ObjectAssociationMap::iterator j = refs->find(key);//直接在map中查找
            if (j != refs->end()) {
...
//关联对象相关函数，objc_setAssociatedObject最终调用的函数
void _object_set_associative_reference(id object, void *key, id value, uintptr_t policy) {
    ...
    {
        AssociationsManager manager;
        AssociationsHashMap &associations(manager.associations());//AssociationsHashMap 是一个C++类，继承与一个unordered_map的泛型类，由名称可知它肯定包含一个无序的哈希表，同时它是一个容器
        disguised_ptr_t disguised_object = DISGUISE(object);//以unsigned long来表示对象，disguised_ptr_t就是无符号长整型，DISGUISE()方法用于取反操作，将object对象地址取反赋给disguised_object变量，（由变量名可知 是做某种伪装），由下面代码看，disguised_object是作为associations容器的key
        if (new_value) {
            // break any existing association.
            AssociationsHashMap::iterator i = associations.find(disguised_object);
            if (i != associations.end()) {
                // secondary table exists
                ObjectAssociationMap *refs = i->second; //ObjectAssociationMap是一个STL map
                ObjectAssociationMap::iterator j = refs->find(key); //STL map对应的iterator中first是key second是value
                if (j != refs->end()) {
    ...
    vector< ObjcAssociation,ObjcAllocator<ObjcAssociation> > elements;
    {
        ...
        if (i != associations.end()) {
            ...
            for (ObjectAssociationMap::iterator j = refs->begin(), end = refs->end(); j != end; ++j) {
                //for循环将需要取消的关联值存入vector中，并在函数最后做释放操作
