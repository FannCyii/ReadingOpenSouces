...
//这个看起来很爽，使用范型来规范数组，这个泛型类用于存储 各种属性、方法、代理的数组列表
/***********************************************************************
...
class list_array_tt {
 ...
 protected:
    ...
    class iterator {
        List **lists;
        List **listsEnd;
        typename List::iterator m, mEnd;//typename用于解决二重定义， 在使用模板的时候使用class 会出现而重定义
...
//重定义 解地址符 第一个使用const *t是调用，这里返回值不能修改，同时后面的const指定其后的代码块中不能写操作。
...
//将新列表添加到旧列表中
    void attachLists(List* const * addedLists, uint32_t addedCount) {
        ...
        if (hasArray()) {
            ...
            //memove的用法(dec,src,len)将内存src开始的len长度的数据存储到内存dec开始的位置 可以有重叠
            memmove(array()->lists + addedCount, array()->lists, 
                    oldCount * sizeof(array()->lists[0])); //将内存中旧列表内容，移动到addedCount的位置上，也就是前面留有addedCount的类容，这里就是方法会覆盖的原因
            //memcpy的用法(dec,src,len)将内存src开始的len长度的数据复制到内存dec开始的位置 不能有重叠
            memcpy(array()->lists, addedLists, 
                   addedCount * sizeof(array()->lists[0]));//将新增列表中的内容添加到，新列表最前面
        }
        else if (!list  &&  addedCount == 1) {
            // 0 lists -> 1 list
            list = addedLists[0]; //如果当前还没有list 并且出入的值长度为1 那么直接就是用出入的内存地址为当前list的头
            
        } 
        else {
            // 1 list -> many lists  //如果当前还没有list 并且出入的值的长度大于一个list值 就进行从新分配
            List* oldList = list;
...
/*
  方法列表、属性列表、代理列表 都是继承的 list_array_tt 底层使用的都是同一个数据结构
 */
...
struct class_rw_t {
    ...
    const class_ro_t *ro; //这有点意思，也有含有方法 属性 变量 代理列表

    method_array_t methods; //方法列表
    property_array_t properties;  //属性列表
    protocol_array_t protocols;   //代理列表

    Class firstSubclass;     //看变量名称 是第一个子类
    Class nextSiblingClass;  //看变量名称 是下一个成员类

    char *demangledName;  //识别码名称
