...
/***********************************************************************
* unattachedCategories 独立的category，创建包含独立category hash映射表，只创建一次
...
* Records an unattached category. 添加一个独立的category到映射表中
* Locking: runtimeLock must be held by the caller.
**********************************************************************/
...
/***********************************************************************
* remethodizeClass  添加一个category到存在的类中。修复该类的方法列表，代理列表，属性列表，并更新该类和其子类的方法缓存区
...
{
    static NXMapTable *remapped_class_map = nil;//定义一个静态映射表
...
static Class realizeClass(Class cls)
{
    ...
    assert(cls == remapClass(cls));//remapClass返回一个存在的cls类实例
    ...
    rw->version = isMeta ? 7 : 0;  // old runtime went up to 6  //是元类 版本设置为7 为啥为7？


    // Choose an index for this class.
    // Sets cls->instancesRequireRawIsa if indexes no more indexes are available
    cls->chooseClassArrayIndex(); //干啥的??
                       ...
                       bool initialize, bool cache, bool resolver)
{
    ...
    //这个加锁机制可以看看 runtimeLock
    runtimeLock.assertUnlocked();

    // Optimistic cache lookup
    if (cache) {  //有方法缓存 则从缓存中获取
        imp = cache_getImp(cls, sel); //这个方法没有公开
        if (imp) return imp;
    }
...
//    auto自动推导类型，&引用，auto& 用于自动推导类型后面类型必须初始化，这种方式比直接使用for循环要快，直接使用引用来访问对象地址，而const可以防止该内存被修改，这个非常规范，可以模仿
