 ...
 *将给定的referrer添加到 这个实体的应用弱指针集合中，不会执行重复的检查（弱指针不会被添加两次）
 ...
 * Performs a lookup.
 *
 执行查找操作，根据给定的referent 返回一个弱引用表中的实体,如果没有对应表实体则放回NULL
...
weak_entry_for_referent(weak_table_t *weak_table, objc_object *referent)
{
    ...
    weak_entry_t *weak_entries = weak_table->weak_entries; //获得链表头

    if (!weak_entries) return nil;

    //计算 referent 在hash表中的位置，并做碰撞检查
                      ...
                      id *referrer_id, bool crashIfDeallocating)
{
    ...
    //先查找到weak对象在链表中的的地址，再将需要引用改对象的指针加入到 引用链表数组中
    weak_entry_t *entry;
    if ((entry = weak_entry_for_referent(weak_table, referent))) {//查找
        append_referrer(entry, referrer);//添加
    } 
    else {
